...
# This @app.before_request block is no longer strictly necessary for chat session management
# because we will now use the session_id sent from the frontend for database operations.
# You can remove it if you don't use Flask's session for other purposes.
# @app.before_request
...
def chat():
    ...
    # CORRECTED: Get the session_id from the JSON body sent by the frontend
    current_session_id = request.json.get('session_id')

    if not current_session_id:
        # It's crucial that the frontend always sends a session_id
    ...
    try:
        ...
        # Use the frontend's session_id to retrieve history for the current conversation
        session_history = get_session_chats(current_session_id)
        
        prompt_parts = [
            "You are a helpful, respectful, and highly accurate healthcare assistant chatbot. "
            "Your primary goal is to provide concise, evidence-based, general health information. "
            "You MUST explicitly state in every relevant answer that you are not a medical doctor, "
            "cannot provide diagnoses, prescribe medication, or offer emergency medical advice. "
            "ALWAYS recommend consulting a qualified healthcare professional for any specific medical concerns or emergencies. "
            "If a query requests a diagnosis, personalized medical advice, or information outside the scope of general public health knowledge, "
            "politely decline and reiterate your role and the need for professional consultation."
            "\n\nWhen providing information, use Markdown for clear formatting, including bolding for headings and bullet points or numbered lists for key information. "
            "Conclude your answers by reiterating the importance of professional medical advice."
        ...
        if response.candidates and response.candidates[0].content.parts:
            ...
            bot_reply = "Sorry, I couldn't generate a response for that. Your query might have triggered safety filters, or it's outside my current capabilities. Please try rephrasing, or consult a medical professional for specific health advice."
        ...
        # Save the chat with the frontend's session_id
...
def clear_history():
    try:
        clear_all_chats()
        # This line clears Flask's session, which is not directly tied to the chat history
        # as per the fix. It's fine to keep for general Flask session management if needed.
        session.pop('session_id', None) # Use .pop for safer removal
